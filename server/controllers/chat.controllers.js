...
const create_chat = async (req, res) => {
    ...
    if (!userId) {
        return res.status(401).json({ success: false, error: "Authentication required (Missing userId)." });
    }
    ...
    try {
        ...
        return res.status(500).json({ success: false, error: "Failed to create new chat session." });
    }
}
...
const send_msg = async (req, res) => {
    ...
    if (!userId || !chatId || !message) {
        return res.status(400).json({ success: false, error: "Missing chatId, userId, or message." });
    }
    ...
    try {
        ...
        const modelReplyText = await callModel(conversationHistory);
...
const get_all_chats = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ success: false, error: "Failed to retrieve chat list." });
    }
}
